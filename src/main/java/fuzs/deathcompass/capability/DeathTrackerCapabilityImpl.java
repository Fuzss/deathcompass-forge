...
public class DeathTrackerCapabilityImpl implements DeathTrackerCapability {
    ...
    public void write(CompoundTag tag) {
        if (tag.contains("LastDeathX", 99) && tag.contains("LastDeathY", 99) && tag.contains("LastDeathZ", 99)) {
            ...
            if (tag.contains("LastDeathDimension")) {
                this.lastDeathDimension = Level.RESOURCE_KEY_CODEC.parse(NbtOps.INSTANCE, tag.get("LastDeathDimension")).resultOrPartial(DeathCompass.LOGGER::error).orElse(Level.OVERWORLD);
            }
    ...
    public void read(CompoundTag tag) {
        if (this.lastDeathPosition != BlockPos.ZERO) {
            ...
            ResourceLocation.CODEC.encodeStart(NbtOps.INSTANCE, this.lastDeathDimension.location()).resultOrPartial(DeathCompass.LOGGER::error).ifPresent((p_9134_) -> {
